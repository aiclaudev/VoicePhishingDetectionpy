...
def random_keyword_evaluate(keywords, model, voice_phishing_test_token, normal_test_token) :

    our_method = {'Accuracy' : [], 'Recall' : [], 'Precision' : [], 'ARL1' : [], 'ARL0' : []}
    search_based = {'Accuracy' : [], 'Recall' : [], 'Precision' : [], 'ARL1' : [], 'ARL0' : []}
    keyword_combinations = list(combinations(keywords, 7))
    print("length of keyword_combinations : ", len(keyword_combinations))

    for idx in tqdm(range(len(keyword_combinations))) :
        keyword = list(keyword_combinations[idx])
        print(keyword)
        keyword_vect = []
        ...
        our_method_result = evaluate(voice_phishing_test_EWMA2, normal_test_EWMA2, 0.425, 9)
...
def main():
    ...
    our_method, search_based = random_keyword_evaluate(keywords, model, voice_phishing_test_token, normal_test_token)
    our_method_result = dictionary_to_result(our_method)
    search_based_result = dictionary_to_result(search_based)
